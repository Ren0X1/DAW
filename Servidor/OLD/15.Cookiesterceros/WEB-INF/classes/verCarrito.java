...
public class verCarrito extends HttpServlet{
	protected void processRequest(HttpServletRequest request,HttpServletResponse response)throws ServletException, IOException{
		...
		if (vectorCookies!=null) {
			...
			out.println("<h1>Hola "+URLDecoder.decode(c.getName().substring(2,c.getName().length()), "utf-8")+" tu carrito es: </h1>");
		...
		out.println("<br>");
		String baseUrl = getServletContext().getRealPath("/")+ "15b.Cookiesterceros";
		String encodedUrl = response.encodeRedirectURL(baseUrl);
		out.println("<a href='"+encodedUrl+"'>Cambiar aplicacion</a>");
		out.println("</body>");
		out.println("</html>");
	}
